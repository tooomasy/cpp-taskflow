...
struct hazptr_domain {
  ...
  template <typename T>
  void retire(hazptr_obj_base<T>* base_obj);
...
hazptr_holder::hazptr_holder() : domain_(get_default_hazptr_domain()) {}

template <typename T>
void hazptr_holder::protect(T* raw_ptr) {
  hptr_ = domain_->acquire_hazptr();
  hptr_->ptr_.store(raw_ptr);
}

void hazptr_holder::release_protection() {
...
hazptr_rec* hazptr_domain::acquire_hazptr() {
...
template <typename T>
void hazptr_domain::retire(hazptr_obj_base<T>* base_obj) {
  retire(new hazptr_retire_obj(base_obj));
}

void hazptr_domain::retire(hazptr_retire_obj* reclaim_obj) {
  // add next avail?
...
void hazptr_domain::reclaim_if_reach_threshold() {
  //
